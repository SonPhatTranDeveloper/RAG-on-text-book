...
class LocalReRankRAG(LocalRAG):
    ...
    def _get_bm25_retriever(self) -> any:
        ...
        if os.path.exists(self.bm25_dir):
            bm25_retriever = BM25Retriever.from_persist_dir(
                persist_dir=self.bm25_dir, similarity_top_k=self.embed_similarity_top_k
            )
        else:
            ...
            bm25_retriever = BM25Retriever.from_nodes(
                nodes, similarity_top_k=self.embed_similarity_top_k
            )
            bm25_retriever.storage_context.persist(persist_dir=self.bm25_dir)
    ...
    def get_query_engine(self) -> any:
        ...
        query_engine = RetrieverQueryEngine.from_retriever(
            retriever=hybrid_retriever, response_mode="compact"
        )
